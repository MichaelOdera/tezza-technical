...
public class EmailDeliveryStrategy implements ChannelDeliveryStrategy {
    ...
    @Override
    public Notification.NotificationChannel getSupportedChannel() { return Notification.NotificationChannel.EMAIL; }

    @Override
    public Mono<Void> send(String customerId, String message) {
        return Mono.fromRunnable(() -> log.info("Sending EMAIL to Client [{}]: {}", customerId, message)).then();
    }
}
